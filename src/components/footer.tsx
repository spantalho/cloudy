import { useTranslation } from "react-i18next";
import { useLang } from "@/hooks/use-lang";
import { Badge } from "./ui/badge";
import * as tooltip from "./ui/tooltip";
import * as avatar from "./ui/avatar";
import * as link from "./common/link";
import * as lucide from "lucide-react";
import { useConfig } from "@/contexts/config-context";
import { useMediaQuery } from "@uidotdev/usehooks";

export default function Footer() {
  const { appConfig } = useConfig();
  const { t } = useTranslation();
  const { lang } = useLang();

  const isDesktop = useMediaQuery("(min-width: 768px)");

  // return (
  //   <footer className="flex flex-col items-center w-full py-4 mt-2">
  //     <div className="flex flex-col md:flex-row items-start md:items-center w-full justify-between text-xs max-w-2xl md:max-w-3xl lg:max-w-4xl gap-5 md:gap-0">
  //       <ul className="flex flex-col md:flex-row items-center md:items-center gap-3 md:gap-5 w-full md:w-auto text-center md:text-left">
  //         {isDesktop && (
  //           <li className="opacity-70">
  //             <span className="mr-1">created by</span>
  //             <tooltip.Tooltip>
  //               <tooltip.TooltipTrigger>
  //                 <link.Link href={appConfig.URLS.app.author_github ?? "#"}>
  //                   {appConfig.APP.author || "[your name]"}
  //                 </link.Link>
  //               </tooltip.TooltipTrigger>
  //               <tooltip.TooltipContent className="flex items-center justify-center p-6">
  //                 <avatar.Avatar className="rounded-md scale-200">
  //                   <avatar.AvatarFallback>
  //                     {appConfig.APP.author?.slice(0, 3) ?? "usr"}
  //                   </avatar.AvatarFallback>
  //                   <avatar.AvatarImage
  //                     src={
  //                       appConfig.URLS.app.author_github
  //                         ? `${appConfig.URLS.app.author_github}.png`
  //                         : ""
  //                     }
  //                   />
  //                 </avatar.Avatar>
  //               </tooltip.TooltipContent>
  //             </tooltip.Tooltip>
  //           </li>
  //         )}

  //         {appConfig.URLS.app.repo && (
  //           <li>
  //             <link.Link href={`${appConfig.URLS.app.repo}#readme`}>
  //               {lang === "en" ? "About" : "Sobre"}
  //             </link.Link>
  //           </li>
  //         )}

  //         <li>
  //           <tooltip.Tooltip>
  //             <tooltip.TooltipTrigger>
  //               <link.Link href={`${appConfig.URLS.app.license}`}>
  //                 {lang === "en" ? "License" : "Licença"}
  //               </link.Link>
  //             </tooltip.TooltipTrigger>
  //             <tooltip.TooltipContent className="max-w-[25em]">
  //               <div>
  //                 <h2 className="font-unbounded tracking-tight font-bold mb-2 capitalize">
  //                   Third-Party Disclaimer
  //                 </h2>
  //                 <p>{t("license_disclaimer")}</p>
  //               </div>
  //             </tooltip.TooltipContent>
  //           </tooltip.Tooltip>
  //         </li>

  //         <li>
  //           <link.Link href={appConfig.URLS.app.services?.weather ?? "#"}>
  //             {t("powered", { service: appConfig.APP.services?.weather ?? "[weather service]" })}
  //           </link.Link>
  //         </li>
  //       </ul>

  //       <ul className="flex flex-row justify-center items-center gap-3 md:gap-5 w-full md:w-auto text-center md:text-right">
  //         <li>
  //           <link.IconLink
  //             href={appConfig.URLS.app.repo || "#"}
  //             icon={<Github size={16} />}
  //           >
  //             Github
  //           </link.IconLink>
  //         </li>

  //         <li>
  //           <Badge>v{appConfig.APP.version}</Badge>
  //         </li>
  //       </ul>

  //       {!isDesktop && appConfig.APP.author && (
  //         <div className="flex w-full justify-center text-xs opacity-70">
  //           <span className="mr-2">&copy; 2025</span>
  //           <a>{appConfig.APP.author}</a>
  //         </div>
  //       )}
  //     </div>
  //   </footer>
  // );

  return (
    <footer className="flex w-full justify-between gap-8 items-baseline text-xs mt-8 mb-4">
      <div>
        <span className="font-unbounded font-bold tracking-tight lowercase text-lg">
          {appConfig.APP.name}
        </span>
        <ul className="flex flex-col gap-2 mt-2 font-bold">
          <li>
            <link.Link href={`${appConfig.URLS.app.repo}#about`}>About & Features</link.Link>
          </li>
          <li>
            <link.Link href="#">Share</link.Link>
          </li>
          <li>
            <link.Link href={`${appConfig.URLS.app.repo}#help`}>Help</link.Link>
          </li>
        </ul>
      </div>
      <div>
        <span className="text-sm">Info</span>
        <ul className="flex flex-col gap-2 mt-2 font-bold">
          <li>
            <tooltip.Tooltip>
              <tooltip.TooltipTrigger>
                <link.Link href={appConfig.URLS.app.license as string || "#"}>License</link.Link>
              </tooltip.TooltipTrigger>
              <tooltip.TooltipContent className="max-w-[25em]">
                <h2 className="font-unbounded tracking-tight font-bold mb-2 capitalize">
                  Third-Party Disclaimer
                </h2>
                <p>{t("license_disclaimer")}</p>
              </tooltip.TooltipContent>
            </tooltip.Tooltip>
          </li>
          <li>
            <link.Link href={appConfig.URLS.app.repo as string || "#"}>
              Source
            </link.Link>
          </li>
          <li>
            <link.Link href={`${appConfig.URLS.app.repo}#credits`}>
              Credits
            </link.Link>
          </li>
        </ul>
      </div>
      <div>
        <ul className="flex flex-col gap-2 items-end text-muted">
          <li>
            <Badge>v{appConfig.APP.version}</Badge>
          </li>
          <li>
            <p>
              Powered by
              <link.Link
                className="ml-1"
                href={appConfig.URLS.app.services?.weather as string}
              >
                {appConfig.APP.services?.weather}
              </link.Link>
            </p>
          </li>
          <li>
            <p>
              2025
              <link.Link
                className="ml-1"
                href={appConfig.URLS.app.author_github as string}
              >
                {appConfig.APP.author}
              </link.Link>
            </p>
          </li>
        </ul>
      </div>
    </footer>
  );
}
